...
export function extractMcpApiKey(request: Request): string {
  ...
  return (url.searchParams.get("api_key") || url.searchParams.get("key") || url.searchParams.get("apiKey") || "").trim();
}
...
export function buildMcpDiscovery(siteUrl = "https://moltad.net") {
  ...
  return {
    ...
    auth: {
      ...
      envStdio: "AGENTIQ_API_KEY",
    },
    modules: MCP_MODULES,
    docs: `${base}/#mcp`,
    docsUrl: "https://github.com/chrisgu/agentiq-mcp",
    stdioPackage: "agentiq-mcp",
  };
}

export async function handleAgentIqMcpHttpRequest(request: Request, opts: HandleAgentIqMcpHttpOptions = {}): Promise<Response> {
  ...
  if (opts.discoveryOnBrowserGet !== false && shouldServeDiscovery(request)) {
    ...
    if (request.method === "HEAD") {
      return withCors(new Response(null, { status: 200, headers: { "Content-Type": "application/json; charset=utf-8", ...CORS_HEADERS } }));
    }
    return withCors(new Response(body, { status: 200, headers: { "Content-Type": "application/json; charset=utf-8", ...CORS_HEADERS } }));
  }

  const apiKey = (opts.apiKey ?? extractMcpApiKey(request)).trim();
  const transport = new WebStandardStreamableHTTPServerTransport({ sessionIdGenerator: undefined, enableJsonResponse: true });

  const { server } = createAgentIqMcpServer({ apiBase, apiKey, fetchImpl: opts.fetchImpl });
